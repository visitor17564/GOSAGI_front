...
export const generateProductCards = async (products) => {
  ...
  productWrap.innerHTML = products
    .map((product) => {
      ...
      return `
      <a href="http://localhost:5500/html/search/detail.html?productId=${product.id}" class="mt-8 flex flex-col rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 items-stretch w-3/12 max-md:w-full max-md:ml-0 px-2 py-2">
...
export const generateSearchCards = async (products) => {
  const productWrap = document.getElementById('product-wrap');
  productWrap.innerHTML = products
    .map((product) => {
      const localesPoint = product.point.toLocaleString();
