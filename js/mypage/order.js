...
async function getAllOrderList() {
  // 주문 목록 전체 조회 API 실행
  const response = await axios.get('http://52.79.88.29:3000/order', {
...
async function getPeriodOrderList() {
  $periodBtns.forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      // 주문 목록 기간 조회 API 실행
      const response = await axios.get(`http://52.79.88.29:3000/order/period?period=${periodType}`, {
        withCredentials: true,
      });
...
async function drawCart(orderId) {
  ...
  try {
    // 문의 글 상세 조회 API 실행
    const response = await axios.get(`http://52.79.88.29:3000/order/${orderId}`, {
      withCredentials: true,
    });
  ...
  document.getElementById('order-modal-order-edit-btn').addEventListener('click', async () => {
    try {
      ...
      const response = await axios.patch(
        `http://52.79.88.29:3000/order/address/${orderId}`,
        {
  ...
  document.getElementById('order-modal-order-cancle-btn').addEventListener('click', async () => {
    if (confirm('주문을 취소하시겠습니까?')) {
      try {
        // 배송지 수정 API 실행
        const response = await axios.patch(
          `http://52.79.88.29:3000/order/refund/${orderId}`,
          {
            status: 4,
          },
          {
  ...
  document.getElementById('order-modal-order-return-btn').addEventListener('click', async () => {
    if (confirm('반품을 신청하시겠습니까?')) {
      try {
        // 배송지 수정 API 실행
        const response = await axios.patch(
          `http://52.79.88.29:3000/order/return/${orderId}`,
          {
            status: 5,
    ...
    if (confirm('새 상품으로 교환을 신청하시겠습니까?')) {
      try {
        ...
        const response = await axios.post(
          `http://52.79.88.29:3000/order/exchange/${orderId}`,
          {
            status: 7,
    ...
    button.addEventListener('click', async function () {
      ...
      if (confirm('구매를 확정하시겠습니까?')) {
        try {
          // 구매 확정 API 실행
          const response = await axios.patch(
            `http://52.79.88.29:3000/order/confirm/${orderId}`,
            {
              status: 3,
            },
            {
