...
export async function getProductByCategory(category, page) {
  ...
  try {
    ...
    const response = await axios.get(`https://https.visitor.run/goods/category/${category}?&page=${page}`);
...
export async function getTotalPageCount(category) {
  try {
    // axios를 사용하여 로그인 API 실행
    const response = await axios.get(`https://https.visitor.run/goods/count/category/${category}`);
